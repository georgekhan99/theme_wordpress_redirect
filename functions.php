...
/**
 * Create DataBase For Collect User IP
 * 
 **/

function create_return_visitor_table() {
    global $wpdb;
...
add_action('init', 'create_return_visitor_table');

/** **/

function create_return_setting_table() {
    global $wpdb;
    $table_name = $wpdb->prefix . "return_setting";
    $charset_collate = $wpdb->get_charset_collate();
    $sql = "CREATE TABLE IF NOT EXISTS $table_name (
      id int(255) NOT NULL AUTO_INCREMENT,
      tb_times int(255)  NOT NULL,
      tb_final_url varchar(255) NOT NULL,
      PRIMARY KEY id (id)
    ) $charset_collate;";
    require_once(ABSPATH . 'wp-admin/includes/upgrade.php');
    dbDelta($sql);
}    
add_action('init', 'create_return_setting_table');
...
add_action('custom_menu', 'my_admin_page_contents');
function my_admin_page_contents() {
		global $wpdb;
		$table_setting_name = $wpdb->prefix .'return_setting';		
		$get_setting  = $wpdb->get_row("SELECT * FROM $table_setting_name WHERE id");
		$get_num_times = $get_setting->tb_times;
		$get_url_re = $get_setting->tb_final_url;	 


	if(isset($_POST['Submit']) && $get_setting->id == 1){		
		$wpdb->update( 
    	$table_setting_name,
    	array( 
        'tb_times' => $_POST['tentacles'],
...
<br>
<label for="tentacles">จำนวนครั้งที่ให้เข้าโดย Redirect:</label>
<input type="number" id="tentacles" name="tentacles"
	   min="0" max="100" value="<?= $get_num_times ?>">
	<p>
		*ค่า 0 เท่ากับฟังก์ชั่นไม่ทำงาน ถ้าไม่ต้องการให้ทำงานใส่ 0
	</p>
<br>	
<br>
...
add_action('init','get_data',1,1);


function check_redirect(){
	global $wpdb;
	$table_name = $wpdb->prefix .'return_vstr';
	$table_setting = $wpdb->prefix .'return_setting';
	$server_ip = $_SERVER["REMOTE_ADDR"];
	$get_ip_addr_on_db = $result = $wpdb->get_row("SELECT * FROM $table_name WHERE user_ip LIKE '$server_ip'", );
	$get_times = $result = $wpdb->get_row("SELECT * FROM $table_name WHERE user_ip LIKE '$server_ip'", );
	$check_times = $get_times->user_count;
	$check_ip = $get_times->user_ip;
	$get_setting = $result = $wpdb->get_row("SELECT * FROM $table_setting WHERE id");
	$get_num_times = $get_setting->tb_times;
	$get_url_re = $get_setting->tb_final_url;
...
function delete_database(){
	if(isset($_POST['Delete'])){
		global $wpdb;
	$wpdb->delete("DELETE FROM return_vstr truncate return_vstr");
	}
}
